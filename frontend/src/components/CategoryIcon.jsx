// frontend/src/components/CategoryIcon.jsx - v3
import React from 'react';

const CategoryIcon = ({ name, className = "w-10 h-10 md:w-12 md:h-12" }) => {
  // Map category names to their icon keys
  const iconMap = {
    'Anti-Aging': 'sparkles',
    'Fertility': 'heart',
    'Weight & Energy': 'fire',
    ...
    'Vision': 'eye',
    'Diabetes': 'drop',
    'High Blood Pressure': 'pulse',  // Changed from 'activity' to 'pulse'
    'Menstrual Health': 'flower',
    'Cancer Support': 'ribbon',
    'Bones & Joints': 'bone'
  };

  const iconKey = iconMap[name] || 'sparkles';
  ...
  const icons = {
    ...
    heart: (
      <svg className={`${className} text-pink-500`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    pulse: (
      <svg className={`${className} text-red-600`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12h3l2-5 4 10 3-5 2 4h3" />
      </svg>
    ),
    flower: (
      <svg className={`${className} text-pink-500`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    ribbon: (
      <svg className={`${className} text-teal-500`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
      </svg>
    ),
    bone: (
      <svg className={`${className} text-amber-600`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H9v-3.708l-1.5-1.5a4 4 0 015.292-5.292l1.5 1.5V21z" />
      </svg>
    )
  };
